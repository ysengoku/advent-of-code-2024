...
int main(int ac, char **av) {
  if (ac != 3 || (strcmp(av[2], "1") && strcmp(av[2], "2"))) {
    std::cerr << "usage: " << av[0] << " <path to file> <part no.> "
              << std::endl;
    return (1);
  }
  std::vector<std::pair<int, int> > data =
      Parser(av[1], std::stoi(av[2])).getData();

  int result = 0;
  std::vector<std::pair<int, int> >::iterator itEnd = data.end();
  for (std::vector<std::pair<int, int> >::iterator it = data.begin();
       it != itEnd; ++it) {
