...
int main(int ac, char **av) {
  if (ac != 3) {
    std::cerr << "usage: " << av[0] << " <path to file1> <path to file2>"
              << std::endl;
    return (1);
  }
  Parser parser = Parser(av[1], av[2]);
  std::vector<std::pair<int, int>> rulesData = parser.getRulesData();
  std::vector<std::vector<int>> updatesData = parser.getUpdatesData();
...
// some page numbers, the ordering rules involving those missing page numbers
// are ignored.
